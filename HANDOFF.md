...
2. **Wire the scorer to the backend.** Make `scoring-app.html` POST events to
   `/games/:id/events` (batched, offline-queued) instead of only holding them in
   memory; pull on reconnect using `sync.ts`.
3. **Follower web view.** A read-only page that subscribes to `GET /stream` and
   renders the scoreboard + play-by-play live, with the graceful-degradation
   behavior (show "last updated", resume via `Last-Event-ID`).
